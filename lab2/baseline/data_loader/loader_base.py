...
class DataLoaderBase(object):
    ...
    def load_cf(self, filename):
        ...
        for l in lines:
            tmp = l.strip()
            inter = [int(i) for i in tmp.split()] # 转化为列表

            if len(inter) > 1:
                user_id, item_ids = inter[0], inter[1:] # 获取对应的userID 和 itemID
                item_ids = list(set(item_ids)) # 将itemID转化为列表

                for item_id in item_ids:
                    user.append(user_id) #获取所有userID
                    item.append(item_id) # 所有itemID
    ...
    def statistic_cf(self):
        self.n_users = max(max(self.cf_train_data[0]), max(self.cf_test_data[0])) + 1 # 获取最大的ID，即user number
        self.n_items = max(max(self.cf_train_data[1]), max(self.cf_test_data[1])) + 1 # cf_train_data[1]对应item，所有
        self.n_cf_train = len(self.cf_train_data[0])  # train data 的数量
        self.n_cf_test = len(self.cf_test_data[0])   # test data 数量
    ...
    # 打乱数据？sample_pos_items不是按照user_id顺序
