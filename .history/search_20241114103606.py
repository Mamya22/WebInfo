...
class BooleanMatch:
    ...
    def print_message(self,_id: int):
        if(self.mode == 'book'):
            # book类型检索
            show_id = str(_id) + ' '*(17-len(str(_id)))# 确保是17个字节，不够用空格补齐
            show_author = "author: " + self.book_keyword[_id]['author'] + ' '*(30-len(self.book_keyword[_id]['author'])) if len(self.book_keyword[_id]['author']) < 30 else ' ' * 0
            show_content = "content: " + self.book_keyword[_id]['content introduction']
